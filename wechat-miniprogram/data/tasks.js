/* 实践任务数据：与网站版 assets/practice-data.js 保持一致（纯文本版） */
module.exports = [
  {
    id: "colabfold-first",
    ...
    steps: [
      {
        title: "准备一条氨基酸序列",
        dur: "2 分钟",
        body: "用一条你熟悉的序列开始。推荐：绿色荧光蛋白 GFP（UniProt P42212），或者你研究里的蛋白。\n格式要 FASTA：第一行 >名字，下面一行是序列。长度 100–400 个氨基酸最合适。",
        expect: "你手里有一条以 > 开头的文本序列。",
        errors: [
          { q: "怎么找到序列？", a: "去 UniProt 搜蛋白名，打开条目后点 FASTA 页签复制即可。" },
          { q: "序列太长怎么办？", a: "想快就截一个结构域，预测会更快。" }
        ],
      ...
      },
      {
        ...
        errors: [
          { q: "打不开 / 白屏？", a: "换 Chrome 或 Edge 重试；网络不稳定时刷新几次。" },
      ...
      },
      {
        title: "连接 GPU 并运行",
        dur: "3 分钟",
        body: "点菜单 Runtime（运行时）→ Change runtime type（更改运行时类型），把 Hardware accelerator（硬件加速器）选成 GPU，保存。然后点 Runtime → Run all（全部运行）。",
      ...
      },
      {
        title: "粘贴你的序列",
        dur: "2 分钟",
        body: "滚动到填写 query_sequence 的 cell，把第 1 步准备的 FASTA 粘贴进去，顺便给 jobname 起个名字（比如 my_first）。",
        expect: "输入框里是 >名字 开头、下面紧跟序列的文本。",
        errors: [
      ...
      },
      {
        title: "运行预测（MSA + 模型）",
        dur: "2–8 分钟",
        body: "回顶部点 Run all 继续。程序先搜索同源序列（MMseqs2），再跑 AlphaFold 模型输出结构。",
      ...
      },
      {
        title: "找到结果文件",
        dur: "2 分钟",
        body: "解压下载的 zip，里面有：\n*_predicted_aligned_error_v1.json（PAE 矩阵）\n*_scores_rank_001_*.json（pLDDT 等分数）\n*_unrelaxed_rank_001_*.pdb（预测结构）",
      ...
      },
      {
        ...
        dur: "5 分钟",
        body: "用 PyMOL 或 ChimeraX 打开 *_unrelaxed_rank_001_*.pdb。ChimeraX：File → Open；PyMOL：File → Open。然后切到 cartoon（卡通）模式看骨架。",
      ...
      },
      {
        title: "检查 pTM 和 PAE",
        dur: "3 分钟",
        body: "在 scores json 里找 ptm 字段：>0.8 说明整体折叠可信。PAE 矩阵图里，对角线附近的蓝色方块表示这些区域之间的相对位置可靠。",
      ...
      },
      {
        title: "保存你的第一个预测",
        dur: "3 分钟",
        body: "把结构截图 + pTM/pLDDT 记录存到你的项目文件夹（比如 ~/Desktop/2026/蛋白设计/colabfold-first/）。命名带日期，方便以后回看。任务完成！",
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        title: "连接 GPU 并安装依赖",
        dur: "3 分钟",
        body: "菜单 Runtime → Change runtime type 选 GPU（可选，小蛋白 CPU 也能跑），然后 Runtime → Run all。",
      ...
      },
      {
        title: "设置采样参数",
        dur: "3 分钟",
        body: "常用参数：num_seq_per_target = 8（采样 8 条序列）、sampling_temp = \"0.1\"（越低越保守）。第一次跑全部用默认也行。如果想保留功能残基，在 fixed residues 里填位置（1 开始）。",
      ...
      },
      {
        title: "挑选候选序列",
        dur: "5 分钟",
        body: "先删掉含 *（终止密码子）的序列；再看有没有保留你需要的固定残基。把 top 1–3 条复制保存。",
      ...
      },
      {
        title: "自洽性检查（关键一步）",
        dur: "10 分钟",
        body: "把选中的序列拿回 ColabFold（任务 1 的流程）预测结构，然后在 ChimeraX 里用 Matchmaker 把预测结构和你的骨架叠合。两者越像，说明这条序列越可能真折叠成目标结构。",
